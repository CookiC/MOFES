...
public class WrapperSubsetEval extends ASEvaluation
  ...
  public SelectedTag getEvaluationMeasure() {
    ...
    */
    ...
    public void setEvaluationMeasure(String name) {
    	SelectedTag evalMeasure=new SelectedTag(name,WrapperSubsetEval.TAGS_EVALUATION);
    	setEvaluationMeasure(evalMeasure);
    }
  /**
   * Returns the tip text for this property
   * 
  ...
  public void buildEvaluator(Instances data) throws Exception {
    ...
    */
    ...
    public double[] evaluateSubset(BitSet subset,String[] statNames) throws Exception {
	    boolean terminate;
	    int i,j,k;
	    int numObj=statNames.length;
	    double[] evalMetric=new double[numObj];
	    double[][] repError = new double[numObj][5];
	    Random Rnd = new Random(m_seed);
	    BitSet subsetCopy=(BitSet)subset.clone();
	    subsetCopy.set(m_classIndex);
	    Instances trainCopy = new Instances(m_trainInstances.attributeFilter(subsetCopy));
	    SelectedTag[] statTags=new SelectedTag[numObj];
	    for(i=0;i<numObj;++i)
	    		...
	    		switch (m_evaluationMeasure.getID()) {
			  		...
			  		case EVAL_MAE:
			  			if (m_trainInstances.classAttribute().isNominal()&& (m_evaluationMeasure.getID() == EVAL_DEFAULT|| m_evaluationMeasure.getID() == EVAL_ACCURACY))
			  				repError[i][j]= 1 - repError[i][j];
			  			else
			  				repError[i][j] = -repError[i][j]; // maximize
			  			break;
			  		default:
		  				if (pluginMetric != null&& !pluginMetric.statisticIsMaximisable(statName)){
				  			repError[i] [j]= -repError[i][j]; // maximize
